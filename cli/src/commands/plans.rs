...
pub async fn run(action: PlansAction, env_override: Option<&str>, output_fmt: &crate::cli::OutputFormat) -> Result<()> {
    let config = PbConfig::load()?;
    let client = make_client(&config, env_override)?;

    match action {
        PlansAction::Create { name, amount, period, description, reference_id, setup_fee, billing_cycles, trial_length, trial_unit } => {
            ...
            if let Some(sf) = setup_fee { body["setup_fee"] = serde_json::json!(sf); }
            if let Some(bc) = billing_cycles { body["billing_cycles"] = serde_json::json!(bc); }
        ...
        PlansAction::Update { id, name, amount, description } => {
            let mut body = serde_json::json!({});
            ...
            match output_fmt {
                crate::cli::OutputFormat::Json => output::print_json(&val),
                crate::cli::OutputFormat::Table => output::print_object_table("Plano Atualizado", &val),
            }
            Ok(())
        }
